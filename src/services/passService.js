...
// Atualiza name/email/plan de um membro já existente, sem mexer em status/valid_until.
// Usado pelo refresh periodico (ver refreshMemberProfiles em syncService.js).
async function updateMemberProfile(circleId, { name, email, plan }) {
  const { rows } = await pool.query(
    `UPDATE members SET name=$1, email=$2, plan=$3, updated_at=NOW()
     WHERE circle_id=$4 RETURNING *`,
    [name, email, plan || 'Membro', circleId]
  );
  return rows[0] || null;
}
